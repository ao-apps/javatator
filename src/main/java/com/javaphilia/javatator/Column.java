...
public class Column {
	...
	public Settings addColumn(JavatatorWriter out) throws SQLException, IOException {
		...
		if(newdefaulttype.length()>0) {
			if(newdefaulttype.charAt(0)=='F') newdefault='F'+settings.getParameter("newdefaultvalue");
			else if(newdefaulttype.charAt(0)=='V') {
			if(newdefaulttype.length()>1) newdefault=newdefaulttype;
			else newdefault='V'+settings.getParameter("newdefaultvalue");
			}
		}
		...
		try {
			...
			if(pvalues!=null) {
				...
				if("bool".equals(columnType)) {
					if("t".equals(columnDefault)) columnDefault="Vtrue";
					else if("f".equals(columnDefault)) columnDefault="Vfalse";
				}
				...
				if(columnDefault!=null && columnDefault.charAt(0)=='F') Util.printEscapedInputValue(out, columnDefault.substring(1));
				out.print("'>");
			} else {
				out.print("<option value=''");
				if(columnDefault==null) out.print(" selected");
				out.print(">[NULL]</option>\n"
					+ "<option value='V'");
				if(columnDefault!=null && columnDefault.charAt(0)=='V') out.print(" selected");
				out.print(">[VALUE-->]</option>\n"
					+ "<option value='F'");
				if(columnDefault!=null && columnDefault.charAt(0)=='F') out.print(" selected");
				out.print(">[FUNCTION-->]</option>\n"
			...
			if(columnExtra!=null) out.print(columnExtra);
			out.print("\">");
			out.endTD();
		} finally {
			out.endTR();
	...
	public Settings processRequest(JavatatorWriter out) throws SQLException, IOException {
		...
		if("edit_column".equals(action)) return printEditColumn(out);
		else if("doedit_column".equals(action)) return editColumn(out);
		else if("delete_column".equals(action)) return confirmDeleteColumn(out);
		else if("dodelete_column".equals(action)) return deleteColumn(out);
		else if("doadd_column".equals(action)) return addColumn(out);
		else if("add_primarykey".equals(action)) return confirmAddPrimaryKey(out);
		else if("doadd_primarykey".equals(action)) return addPrimaryKey(out);
		else if("drop_primarykey".equals(action)) return confirmDropPrimaryKey(out);
		else if("dodrop_primarykey".equals(action)) return dropPrimaryKey(out);
		else if("add_index".equals(action)) return printAddIndex(out);
		else if("doadd_index".equals(action)) return addIndex(out);
		else if("add_uniqueindex".equals(action)) return printAddUniqueIndex(out);
		else if("doadd_uniqueindex".equals(action)) return addUniqueIndex(out);
		else {
