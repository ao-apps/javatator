...
public class Main extends HttpServlet {
	...
	private static void printLoginForm(JavatatorWriter out, Settings settings) throws IOException {
        ...
        try {
            ...
            out.printTD("&nbsp;", "rowspan=5 width=12 nowrap");
            out.startTD();
            ...
            if(configHostnamesLen>1) {
                ...
                for(int d=0;d<configHostnamesLen;d++) {
                    ...
                    out.print('\'');
                    if(hostname.equals(settingsHostname)) out.print(" selected");
                    out.print(">");
                    out.print(hostname);
                }
                out.print("</select>");
            } else if(configHostnamesLen==1) out.print(configHostnames.get(0));
            else {
            ...
            if(configPort!=null && configPort.length()>0) out.print(configPort);
            else {
            ...
            if(configUsername!=null && configUsername.length()>0) out.print(configUsername);
                ...
                if(username==null) username=settings.getUsername();
            ...
            if(configPassword!=null && configPassword.length()>0) out.print("XXXXXXXXXXXX");
                ...
                if(password==null) password=settings.getPassword();
            ...
            if(configDatabase!=null && configDatabase.length()>0) out.print(configDatabase);
                ...
                if(database==null) database=settings.getDatabase();
	...
	private void printTopFrame(JavatatorWriter out, Settings settings, String action) throws IOException {
        ...
        out.print("</head>\n"
              + "<body onLoad='");
        if(isConnected) out.print("drawMenu(top.left_frame.window.document);");
    ...
    private void showOptions(JavatatorWriter out, Settings settings) {
        ...
        if(settings.useMultiLine()) out.print(" checked");
